---
layout: default
title: 3.1 Repeating Actions with Loops 
nav_order: 17
---

# 3.1 Repeating Actions with Loops 
...
museum-visit dataset (`exhibit-visits-01.csv`).

![Line graphs showing average, maximum and minimum museum visits across all museums over a 40-day period.](fig/2.3_3_subplots.png)

Imagine that the 6 cities containing the museums sent you their data separately.
We have 6 CSV files, each with data for the visits to the digital exhibit in 10 museums (rows) over 40 days (columns).
We want to create plots for all of
...
1. **Difficult to maintain**. If we want to decorate each printed element with an
...
1. **Fragile**. If we use it with a list that has more elements than what we initially
...
![Loop variable 'num' being assigned the value of each element in the list odds in turn and then being printed"](fig/3.1_loops_image_num.png)
...
## Challenge: Understanding the loops
...
The built-in function `enumerate` takes a sequence (e.g. a [list](1.3_lists.md)) and
