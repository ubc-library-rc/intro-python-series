---
layout: default
title: 5.2 Writing reliable code
nav_order: 24
---
...
For example, let's return to our function that loads a file containing book-search data and determines which month (column) was the busies, across all the books (rows).
...
def print_busiest_month(data_filename):
    ...
    assert isinstance(data_filename, str), "input must be a string"
    assert "VALUES" not in data_filename, "wrong data file"

    df = pandas.read_csv(data_filename, index_col=0)
    assert df.shape == (8, 61), "data is wrong shape"
    ...
    assert "search" in search_name, "file name doesn't match pattern"

    assert top_month > numpy.datetime64("2019-12-31"), "impossible date - before 2020"
    assert top_month < numpy.datetime64("2027-01-01"), "impossible date - after 2026"

    print(f"{search_name}'s top month was {top_month}")

```

The preconditions catch invalid inputs.

```python
print_busiest_month("2026-02-18_google-trends-search-1_VALUES.csv")
```

```output
Traceback (most recent call last):
  File "/home/kendra/teaching/intro-python-series-code/5.2_reliable-code.py", line 41, in <module>
    print_busiest_month("2026-02-18_google-trends-search-1_VALUES.csv")
  File "/home/kendra/teaching/intro-python-series-code/5.2_reliable-code.py", line 11, in print_busiest_month
    assert "VALUES" not in data_filename, "wrong data file"
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
AssertionError: wrong data file
```

The post conditions make sure the output is as expected.

```python
df = pandas.read_csv("2026-02-18_google-trends-search-1.csv", index_col=0)
df = df.rename({"2023-05-01": "2027-02-01"}, axis=1)
df.to_csv("2026-02-18_google-trends-search-1_TAMPERED.csv")

print_busiest_month("2026-02-18_google-trends-search-1_TAMPERED.csv")
```

```output
Traceback (most recent call last):
  File "/home/kendra/teaching/intro-python-series-code/5.2_reliable-code.py", line 48, in <module>
    print_busiest_month("2026-02-18_google-trends-search-1_TAMPERED.csv")
  File "/home/kendra/teaching/intro-python-series-code/5.2_reliable-code.py", line 30, in print_busiest_month
    assert top_month < numpy.datetime64("2027-01-01"), "impossible date - after 2026"
           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
AssertionError: impossible date - after 2026
```
...
![Graph showing three number lines and, at the bottom, the interval that they overlap.](fig/5.2_python-overlapping-ranges.svg)
...
1. Writing tests helps programmers figure out what the function is actually supposed to do.
