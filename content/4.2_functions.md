...
def visualize(data_filename):
    ...
    data = numpy.loadtxt(fname=data_filename, delimiter=",")

    fig3, axs = matplotlib.pyplot.subplots(1, 3, figsize=(10, 3))
    ...
    fig3.savefig(f"4.2_visualize-mean-max-min_{data_filename[-4]}.png")
```

and another function called `print_week_with_busiest_day` to see which week contained the given city's busiest day.

```python
def print_week_with_busiest_day(data_filename):
    i_city_no = len("exhibit-visits_city-")
    city_number = int(data_filename[i_city_no : i_city_no + 1])
    data = numpy.loadtxt(fname=data_filename, delimiter=",")
...
for file_path in file_paths:
    print(file_path)
    visualize(file_path)
    print_week_with_busiest_day(file_path)
```
...
```python
data = numpy.loadtxt(fname='exhibit-visits_city-*_varied-peak.csv', delimiter=',')
print(offset_mean(data, 0))
```

```output
[[-1.121875  5.878125  3.878125  3.878125 -0.121875 -0.121875 -3.121875
  -1.121875  0.878125  0.878125 -3.121875 -3.121875 -0.121875 -3.121875
  -4.121875 -3.121875  0.878125 -4.121875 -0.121875  4.878125 10.878125
   3.878125  6.878125 -0.121875  5.878125 -0.121875  3.878125 -0.121875
  -1.121875  2.878125  0.878125 -3.121875 -3.121875 -2.121875  0.878125
  -3.121875 -0.121875 -0.121875 -0.121875 -0.121875]
 [10.878125 -3.121875  4.878125 -2.121875  4.878125 -0.121875  3.878125
  ...]...]
```
...
```output
original min, mean, and max are: 0.0 7.121875 20.0
min, mean, and max of offset data are: -7.121875 -2.2204460492503132e-17 12.878125
```

That seems almost right:
the original mean was about 7.1,
so the lower bound from zero is now about -7.1.
...
```output
std dev before and after: 4.2976181175594235 4.2976181175594235
```
<!-- 
...
```python
numpy.loadtxt('exhibit-visits_city-7_varied-peak.csv', delimiter=',')
```
...
```python
numpy.loadtxt("exhibit-visits_city-7_varied-peak.csv", ",")
```
