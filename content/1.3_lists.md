...
Lists are built into the language, so we do not have to load a library
to use them.
...
For example, you could represent the products on the shelves of an artist's supply closet as a nested list called `supplies`:

![](fig/1.3_lists_art-supplies-matrix-0.png)

To store the contents of the shelf in a nested list, you write it this way:

```python
supplies = [['pencils', 'paint', 'thread', 'yarn'],
     ['scratch knives', 'glue gun', 'markers', 'palette'],
     ['pastels', 'paint brushes', 'charcoal', 'sewing needles']]
```

Here are some visual examples of how indexing a list of lists `supplies` works. First,
you can reference each row on the shelf as a separate list. For example, `supplies[2]`
represents the bottom row, which is a list of the baskets in that row.

![](fig/1.3_lists_art-supplies-matrix-1.png)
...
![](fig/1.3_lists_art-supplies-matrix-2.png)
...
## Challenge: Slicing From the End
...
## Challenge: Non-Continuous Slices
