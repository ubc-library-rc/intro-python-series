...
In order to repeat an action over multiple files, we need a way to get a list of the relevant files.
We'll start by getting a list of files that begin with `"2026-02-18_google-trends-search-` and end with `VALUES.csv`.
...
In our case, the "something" we want to do is generate a set of plots for each file.
...
filenames_original = glob.glob("2026-02-18_google-trends-search-*VALUES.csv")
filenames_sorted = sorted(filenames_original)
