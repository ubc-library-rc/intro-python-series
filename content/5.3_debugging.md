...
1. *Check conservation laws.*
  ...
  Similarly,
  if we are analyzing the museum-visit data,
  the number of records (number of museums) should either stay the same or decrease
  as we move from one analysis to the next
  (since we might throw away outliers or records with missing values).
  If "new" museums start appearing out of nowhere as we move through our pipeline,
...
1. *Visualize.*
  ...
  (just as we did in previous tutorials).
...
we should *be humble* and [ask for help](https://ubc-library-rc.github.io/intro-python-series/content/1.2_fundamentals.html#getting-help).
